...
构建产物中的 Service Worker 带有应用资源指纹。分包变化时，已有浏览器会重新缓存
完整应用及延迟加载页面；尚未关闭的旧标签页仍可使用原来的带摘要资源。原始响应在
返回浏览器前复制给缓存，避免缓存写入与浏览器读取响应体互相争用。
...
验证启动期间立即关闭后进程退出。随后通过安装程序的真实 WebView2 创建思维图与
Flow，在文字尚未结束编辑时发出原生关闭请求，核对 Markdown，再启动并检查可见内容；
最后重装同一版本并再次验证内容保留。调试连接只在临时 CI runner 为该应用开启，
检查结束恢复原有设置，并再次验证未开启调试连接时的启动关闭；产品不内置测试入口。
这覆盖安装、编辑、保存、重开和同版重装，不代表所有 Windows 版本、未来
版本迁移或发行者信任已验证。发布以对应提交的实际运行结果为准，逐平台说明系统、
架构与签名状态，附同批校验和，不覆盖已有版本资产。
